...
export function TerminalLauncher({ onOpen }: { onOpen: () => void }) {
  return (
    <motion.button
      ...
      onClick={onOpen}
      className="fixed bottom-6 right-6 z-40 flex items-center gap-2.5 font-mono text-xs px-4 py-2.5 rounded-full border border-border bg-panel/90 backdrop-blur-md text-ink hover:border-accent hover:text-accent shadow-lg transition-colors focus-ring"
